...
// =============================================================================
// TTS Offscreen Document Management
// =============================================================================

const TTS_OFFSCREEN_PATH = "offscreen/tts-player.html";
let creatingOffscreen: Promise<void> | null = null;

/**
 * Check if the TTS offscreen document already exists
 */
async function hasOffscreenDocument(): Promise<boolean> {
  const contexts = await chrome.runtime.getContexts({
    contextTypes: ["OFFSCREEN_DOCUMENT"],
    documentUrls: [chrome.runtime.getURL(TTS_OFFSCREEN_PATH)],
  });
  return contexts.length > 0;
}

/**
 * Create the TTS offscreen document if it doesn't exist
 */
async function ensureTTSOffscreenDocument(): Promise<void> {
  const exists = await hasOffscreenDocument();
  if (exists) return;

  // Prevent race conditions by reusing the same promise if already creating
  ...
  creatingOffscreen = chrome.offscreen.createDocument({
    url: TTS_OFFSCREEN_PATH,
    reasons: ["AUDIO_PLAYBACK"],
    justification: "Playing TTS audio with streaming support",
  });
...
Return ONLY valid HTML, no explanations or markdown.`;
      ...
      return true;
    } else if (message.action === "playTTS") {
      // Streaming TTS playback via offscreen document
      (async () => {
        try {
          const { text, voice = "alba", serverUrl = "http://localhost:8000" } = message;
          ...
          // Send play command to offscreen document
          const response = await chrome.runtime.sendMessage({
            target: "offscreen-tts",
            action: "play",
            text,
            voice,
            serverUrl,
          ...
          if (!response.success) {
            throw new Error(response.error || "Failed to start TTS playback");
          }

          console.log("[TTS] Streaming playback started");
          sendResponse({ success: true });
        } catch (error) {
          console.error("[TTS] Error in playTTS handler:", error);
          sendResponse({
            success: false,
            error: error instanceof Error ? error.message : String(error),
          });
      ...
      (async () => {
        try {
          ...
          console.error("[TTS] Error stopping TTS:", error);
          sendResponse({ success: true }); // Still report success - playback should be stopped
        }
      })();
      return true;
      ...
      return true;
    } else if (message.action === "getTTSState") {
      ...
      (async () => {
        try {
          ...
          if (!hasOffscreen) {
            sendResponse({
              success: true,
              state: { playing: false, paused: false, currentTime: 0, duration: 0 },
            });
            return;
          }
          const response = await chrome.runtime.sendMessage({
      ...
      // Forward playback events to content scripts (from offscreen document)
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        if (tabs[0]?.id) {
